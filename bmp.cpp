...
bool AUX_RGBImageRec::loadFile(const char* Filename)
{
	...
	LPCWSTR ptr;
	mbstowcs(search_path_t, Filename, strlen(Filename) + 1);
	ptr = search_path_t;
	HANDLE DIBHandle = LoadImage(0, ptr, IMAGE_BITMAP, 0, 0, LR_DEFAULTCOLOR | LR_CREATEDIBSECTION | LR_LOADFROMFILE);  // use windows to get header info of bitmap - assume no errors in header format 
